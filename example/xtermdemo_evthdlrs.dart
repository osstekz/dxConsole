...
//	assert(evtInput[iKA_EVENTTYPE] == KEY_EVENT);
		...
		switch (iKey) {
			...
			default:
				...
				XTerm.processText(XTerm.ANSICMD_BELL);
				bHandled = true;
				break;
		}
	}
...
//		}
				...
				switch (iKey) {
					case VK_ESCAPE:
						...
						ciEvents.start(iCONSOLEPARMS_MAIN, sSTDIN_CMD_HELP).whenComplete(() {
							...
							XTerm.processText(XTerm.ANSICMD_CURSOR_SHOW);
							//_xwinmgr.clear();
		...
		assert(print("evthdlrMainMenu invalid iEvtType:$iEvtType"));
	}
}
...
//	}
	...
	if (_cmd == sSTDIN_CMD_FORMSMODE) {
		...
		sb.clear();
		sb.writeAll([sPostAnsiReset, XTerm.ANSICMD_ERASE2J_CLEARSCREEN_HOME_CURSOR]);
...
//show command prompt
	...
	XTerm.processText(resetBackground() + xpen.down + "\n>");
}
