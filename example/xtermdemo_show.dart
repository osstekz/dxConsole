	...
	if (!bResult) {
		...
		try {
			...
			_listFutures.add(xwinTab1.dispatch(new XControlEventCreate(false)).then((bResult) {
				...
				//XTerm.processText(XTerm.ANSICMD_BELL + "\x1B[0;30;40mXTerm.CreateWindow failed\n");
			...
			//			XTerm.parseText(XTerm.ANSICMD_BELL);
		}
	}
	if (bResult) {
...
bool showMainMenu([bool bAttrsChanged = false]) {
	...
	if (!bResult) {
		...
		try {
			...
			xwinMain = new XTabbedWindow(ts, iPOSX, iPOSY, iMAXROWS_MAINMENU, iMAXCOLS_MAINMENU)
					..nxsLabel = "XTerm: Dart Server-side GUI (forms mode)"
					..border(iBORDERTYPE);
	...
	if (bResult) {
		...
		XTerm.processText(sPostAnsiReset + XTerm.ANSICMD_ERASE2J_CLEARSCREEN_HOME_CURSOR);
		if (bAttrsChanged) {
		...
		Future.wait(_listFutures, eagerError: true).then((onValue) {
			...
			if (bShowRuler) {
				final int MAXROWSRULER = 30,
						RULERCOLOR = 31;
				int iLineNo;
				XTerm.writeText("*....:....1....:....2....:....3....:....4....:....5....:....6....:....7....:....8....:....9", 0, 0, RULERCOLOR);
				for (int i = 1; i < MAXROWSRULER; i++) {
					for (int i2 = 0; i2 < 4; i2++) {
						XTerm.writeText(".", i++, 0, RULERCOLOR);
					}
					iLineNo = i % 10;
					XTerm.writeText(iLineNo == 5 ? "-" : (i ~/ 10).toString(), i, 0, RULERCOLOR);
				}
			}
			//--------------------------------------------------------
...
//			XTerm.writeText("F1  :Help", iNAVROW1, iCOL1, _colorFGBG);
//			XTerm.writeText("ESC :Exit", iNAVROW2, iCOL1, _colorFGBG);
//			XTerm.writeText("-> :NextTab", iNAVROW1, iCOL1 + 10, _colorFGBG);
//			XTerm.writeText("<- :PrevTab", iNAVROW2, iCOL1 + 10, _colorFGBG);
			//--------------------------------------------------------
			//XTERM supports interpreted ansi control codes
			...
			sb.writeAll([xpen.down, xpen.getANSICMD_MOVE_CURSOR_ROW_COL_H(iNAVROW1, iCOL1 + 2), " :Help", //
				...
				XTerm.ANSICMD_BELL + XTerm.ANSICMD_CURSOR_HIDE]);
			XTerm.processText(sb.toString());
...
bool showHelp() {
	...
	//pen.write('${XTerm.ANSICMD_ERASE2J_CLEARSCREEN_HOME_CURSOR}');
	...
	sb.clear();
	sb.writeAll([sPreAnsiFGColor, XTerm.ANSICMD_ERASE2J_CLEARSCREEN_HOME_CURSOR, "==============", xpen.down, " XTerm: Dart Server-side GUI!!! ", sPreAnsiFGColor, "=============="]);
...
//	sb.write(xpen.getANSICMD_MOVE_CURSOR_CURRENTROW_COL_G(iCOMMANDCOL));
//	sb.writeAll([sPreAnsiFGColor, sSTDIN_CMD_TESTXWIN, sPostAnsiReset, "- XTerm test panel\n"]);
	sb.write(xpen.getANSICMD_MOVE_CURSOR_CURRENTROW_COL_G(iCOMMANDCOL));
